...
export default function FormularioPelicula(props: FormularioPeliculaProps){
    ...
    // Intercepto el onSubmit para poder trabajar con generos seleccionados
    const onSubmit: SubmitHandler<PeliculaCreacion> = (data) => {
        data.generosIds = generosSeleccionados.map(x => x.llave); // Toma los generos seleccionados, Me trae solo los id
        data.cinesIds = cinesSeleccionados.map(x => x.llave);
        data.actores = actoresSeleccionados;

        props.onSubmit(data); // Le paso los ids al componente padre para que los procese
    }
