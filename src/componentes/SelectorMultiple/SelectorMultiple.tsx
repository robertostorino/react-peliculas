        ...
        <div className={styles.div}>
            <ul>
                {props.noSeleccionados.map(item => 
                    <li className={styles.li} key={item.llave} onClick={() => seleccionar(item)}>
                        {item.descripcion}
                    </li>)}
            </ul>
...
interface SelectorMultipleProps {
    ...
    onChange(seleccionados: SelectorMultipleModel[], noSeleccionados: SelectorMultipleModel[]): void; //dispara un evento cada vez que el usuario interactúe con el componente
}